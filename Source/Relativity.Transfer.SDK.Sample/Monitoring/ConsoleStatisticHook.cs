namespace Relativity.Transfer.SDK.Sample.Monitoring
{
	...
	internal static class ConsoleStatisticHook
	{
		...
		private static string BuildStatisticsMessage(TransferJobStatistics statistics)
		{
			...
			var totalMegaBytes = statistics.TotalBytes.HasValue ? $"{ByteSize.FromBytes((double)statistics.TotalBytes).MegaBytes:F}" : notKnown;
			...
			var estimatedTime = statistics.EstimatedTime.HasValue ? statistics.EstimatedTime.Value.ToString(@"h\:mm\:ss") : notKnown;

			return  $"{statistics.Timestamp:T}: {bytesStatistic} MB, Items: {transferredItems}, Skipped: {statistics.CurrentItemsSkipped}, Failed: {statistics.CurrentItemsFailed}, ETA: {estimatedTime}";
		}
