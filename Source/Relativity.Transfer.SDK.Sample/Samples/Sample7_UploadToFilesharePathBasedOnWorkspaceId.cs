namespace Relativity.Transfer.SDK.Sample.Samples
{
	...
	internal class Sample7_UploadToFilesharePathBasedOnWorkspaceId : SampleBase
	{
		public Sample7_UploadToFilesharePathBasedOnWorkspaceId(ConsoleHelper consoleHelper) : base(consoleHelper) { }
		...
		public override async Task ExecuteAsync()
		{
			...
			var fileshares = await filesharesRetriever.GetWorkspaceFilesharesAsync(workspaceId, relativityInstanceAddress, clientId, clientSecret).ConfigureAwait(false);
		...
		private class WorkspaceFilesharesRetriever
		{
			...
			///},
			/// </returns>
			public async Task<IEnumerable<FileshareInfo>> GetWorkspaceFilesharesAsync(int workspaceID, string instanceUrl, string clientId, string clientSecert)
			{
				var tokenRetriever = new BearerTokenRetriever(instanceUrl);
				var token = await tokenRetriever.RetrieveTokenAsync(clientId, clientSecert);
